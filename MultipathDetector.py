...
class MultipathDetector():
    ...
    def computeDistance(coord1, coord2):
        ...
        if distance <= 0.0:
            ...
            log.warning('distance between two coordinates is less than or equal to zero - something is wrong!  (MultipathDetector.distanceForumula)')
            return None
        else:
            return distance
    ...
    def multipathDetect(coord1, coord2, coord3):
        ...
        if dist1_2 < (gpsDistance - outlierMultiplier*linearTolerance):
            log.warning('dist1_2 < (gpsDistance - outlierMultiplier*linearTolerance)')
            outlierFlag = True
        elif dist1_2 > (gpsDistance + outlierMultiplier*linearTolerance):
            log.warning('dist1_2 > (gpsDistance + outlierMultiplier*linearTolerance)')
            outlierFlag = True
        elif dist2_3 < (gpsDistance - outlierMultiplier*linearTolerance):
            log.warning('dist2_3 < (gpsDistance - outlierMultiplier*linearTolerance)')
            outlierFlag = True
        elif dist2_3 > (gpsDistance + outlierMultiplier*linearTolerance):
            log.warning('dist2_3 > (gpsDistance + outlierMultiplier*linearTolerance)')
            outlierFlag = True
        elif dist1_3 < (2*gpsDistance) - outlierMultiplier*linearTolerance:
            log.warning('dist1_3 < (2*gpsDistance) - outlierMultiplier*linearTolerance')
            outlierFlag = True
        elif dist1_3 > (2*gpsDistance) + outlierMultiplier*linearTolerance:
            log.warning('dist1_3 < (2*gpsDistance) - outlierMultiplier*linearTolerance')
            outlierFlag = True
        ...
        if dist1_2 < (gpsDistance - linearTolerance):
            log.warning('dist1_2 < (gpsDistance - linearTolerance)')
            multipathFlag = True
        elif dist1_2 > (gpsDistance + linearTolerance):
            log.warning('dist1_2 > (gpsDistance + linearTolerance)')
            multipathFlag = True
        elif dist2_3 < (gpsDistance - linearTolerance):
            log.warning('dist2_3 < (gpsDistance - linearTolerance)')
            multipathFlag = True
        elif dist2_3 > (gpsDistance + linearTolerance):
            log.warning('dist2_3 > (gpsDistance + linearTolerance)')
            multipathFlag = True
        elif dist1_3 < (2*gpsDistance) - linearTolerance:
            log.warning('dist1_3 < (2*gpsDistance) - linearTolerance')
            multipathFlag = True
        elif dist1_3 > (2*gpsDistance) + linearTolerance:
            log.warning('dist1_3 > (2*gpsDistance) + linearTolerance')
            multipathFlag = True
        ...
        if dotProduct > (1 + outlierMultiplier*dotProductTolerance):
            log.warning('dotProduct > (1 + outlierMultiplier*dotProductTolerance)')
            outlierFlag = True
        ...
        if dotProduct > (1 + dotProductTolerance):
            log.warning('dotProduct > (1 + dotProductTolerance)')
            multipathFlag = True
    ...
    def altitudeCheck(verticalTolerance,alt1, alt2, alt3, debug = False):
        ...
        if abs(alt1 - alt2) > verticalTolerance:
            log.warning('abs(alt1 - alt2) > verticalTolerance')
            altMultipath = True
        elif abs(alt2 - alt3) > verticalTolerance:
            log.warning('abs(alt2 - alt3) > verticalTolerance')
            altMultipath = True
        elif abs(alt1 - alt3) > verticalTolerance:
            log.warning('abs(alt1 - alt3) > verticalTolerance')
            altMultipath = True
        return altMultipath
    ...
    def multipathQueueHandler(listOfQueues):
        ...
        if len(listOfQueues) != 3:
            ...
            log.warning('ERROR: List of queues contains an incorrect number of queues (exactly 3 needed)')
        ...
        if len(queue1) != len(queue2) or len(queue1) != len(queue3) or len(queue2) != len(queue3):
            ...
            log.warning('ERROR: Queues are not of equal length')

        if len(queue1) != 10:
            print "ERROR: Queues are not of length 10"
            log.warning('ERROR: Queues are not of length 10')
        ...
        if multipathCounter >= 3:
            log.warning('multipathCounter')
            return True
        elif finalOutlierFlag == True:
            log.warning('finalOutlierFlag')
            return True
        else:
            return avgMultipath
=======
